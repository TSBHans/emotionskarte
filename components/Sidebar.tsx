...
export default function Sidebar(props: SidebarProps) {
  const { tab, onTabChange, metric, onMetricChange, places, onPlacesChange, filters, onFiltersChange } = props;
  ...
  const sidebarContent = (
    <div className="flex h-full w-[23rem] flex-col overflow-hidden rounded-r-3xl bg-panel-gradient text-slate-100 shadow-sidebar ring-1 ring-white/10 backdrop-blur-xl">
      <header className="border-b border-white/5 px-6 pb-6 pt-7">
        <p className="text-[11px] uppercase tracking-[0.45em] text-primary-100">Emotionale Stadt</p>
        <h2 className="mt-3 font-display text-2xl font-semibold leading-tight">
          Wo ist Berlin vital?
        </h2>
        <p className="mt-2 text-sm text-slate-300">
          Verteilung der Emotion <span className="text-primary-50">{METRIC_LABELS[metric]}</span> nach Ort und Intensität.
        </p>
      </header>
          ...
          <div className="flex flex-col gap-7">
            ...
            <section className="space-y-4">
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-400">
                  Wähle eine Kennzahl aus, um die Karte entsprechend zu färben. Die Intensität reicht von 1 bis 5.
                </p>
              </div>
            ...
            <section className="space-y-5 rounded-3xl border border-white/5 bg-slate-900/30 p-5">
              <div className="space-y-2">
                <h3 className="text-sm font-semibold uppercase tracking-[0.3em] text-slate-300">
                  Filter</h3>
                <p className="text-xs text-slate-400">
                  Verfeinere die Anzeige für dein Analyseziel. Filter wirken sich auf Opazität und Tooltip aus.
                </p>
              </div>
              ...
              <FilterSlider
                ...
                onChange={(value) => updateFilter({ minValue: Math.min(value, filters.maxValue) })}
              />
              <FilterSlider
                ...
                onChange={(value) => updateFilter({ maxValue: Math.max(value, filters.minValue) })}
              />
              <FilterSlider
            ...
            <section className="grid gap-4 rounded-3xl border border-white/5 bg-slate-900/30 p-5">
              ...
              <p className="text-xs text-slate-400">
                Darstellung exemplarisch: reale Werte hängen von der Auswahl auf der Karte ab.
              </p>
            </section>
          </div>
        )}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.35em] text-slate-400">
        <span>{label}</span>
        <span className="rounded-full bg-slate-900/60 px-3 py-1 text-xs font-semibold text-primary-50">{value}</span>
      </div>
  ...
  return (
    <div className="space-y-8 text-sm text-slate-200">
      <section className="space-y-3 rounded-3xl border border-white/5 bg-slate-900/30 p-6">
        <h3 className="text-lg font-semibold text-primary-100">Über die Daten</h3>
        <p>
          Die Emotionen und Umweltwahrnehmungen stammen aus einer Befragung Berliner Bürger:innen. Alle Antworten
          wurden anonymisiert und auf H3-Hexagone aggregiert.
        </p>
        <p>
          Drei Kontexte stehen zur Verfügung: Drinnen, Draußen und ÖPNV. Bei der Visualisierung werden sie mit einer
          OR-Logik kombiniert, sodass bereits ein Kontext genügt, um Daten für ein Hexagon anzuzeigen.
        </p>
      </section>
      <section className="grid gap-3 rounded-3xl border border-white/5 bg-slate-900/30 p-6">
        <h4 className="text-base font-semibold text-primary-100">Variablen & Skalen</h4>
        <p>
          Skala 1 (niedrig) bis 5 (hoch) für Emotionen wie Stress, Glück oder Energie sowie für Umweltfaktoren wie
          Sicherheit oder Urbanes Grün. Zusätzlich wird die Anzahl der Teilnehmer:innen je Hexagon erfasst.
        </p>
      </section>
      <section className="grid gap-3 rounded-3xl border border-white/5 bg-slate-900/30 p-6">
        <h4 className="text-base font-semibold text-primary-100">Methodik</h4>
        <p>
          Teilnehmende ordneten Wahrnehmungen konkreten Orten zu. Pro Ort und Kontext wurden Mittelwerte gebildet,
          anschließend in H3-Zellen aggregiert und mit Feature States dynamisch in die Karte gespielt.
        </p>
      </section>
      ...
      <section className="space-y-2 rounded-3xl border border-white/5 bg-slate-900/30 p-6 text-xs text-slate-400">
        <p>
          Datenschutz: Alle Daten wurden aggregiert, sodass keine Rückschlüsse auf Einzelpersonen möglich sind.
        </p>
...
function RadarPlaceholder() {
  return (
    <div className="relative flex h-28 w-28 items-center justify-center">
      ...
      <span className="text-[11px] uppercase tracking-widest text-primary-100">Demo</span>
    </div>
  );
}
